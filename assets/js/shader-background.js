...
(function () {
    ...
    function mountShaderBackground(canvas) {
        var gl = canvas.getContext("webgl", { antialias: false });
        if (!gl) {
            console.warn("[shader-plasma] WebGL não disponível neste navegador.");
            return;
        }
        ...
        var flatColors = [];
        UNIFORMS.colors.forEach(function (c) { flatColors.push(c[0], c[1], c[2]); });
        ...
        function render(now) {
            ...
            var pointerSettling = Math.abs(targetX - mouseX) > 0.001 ||
    ...
    function boot() {
        var canvas = document.getElementById("shader-plasma-bg");
        if (!canvas) return;
        mountShaderBackground(canvas);
    }
